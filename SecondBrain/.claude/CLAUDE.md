...
6. **Cấu hình graph baseline** (`.obsidian/graph.json`): vault mở từ `E:\Claude` (index cả `Projects/` lẫn `SecondBrain/`) → **graph hiện cả tri thức SecondBrain lẫn cụm Projects**, giữ gọn bằng ẩn operational + orphan. Filter ẩn config + node operational + note nguồn fan-trùng-MOC (`-path:".obsidian" -path:".claude" -file:"CHANGELOG" -file:"README" -file:"CLAUDE" -file:"00_HOME" -file:"tag-system" -file:"flashcards" -file:"track" -path:"60_Daily" -path:"99_Meta/templates" -path:"99_Meta/guides" -path:"30_Resources/courses" -path:"sources"`), **không** loại `Projects/`. Hai filter cuối ẩn source/course note (vd `claude-101-anthropic-academy` fan 19 atomic) theo mục 5 — xem provenance bằng Local Graph + Backlinks. **Color group**: 5 tầng PARA + `Projects/` (cam). `showOrphans:false` (đã thử true thấy rối → tắt), `hideUnresolved:true`. Thông số lực/hiển thị để user tự tinh chỉnh trong app.
